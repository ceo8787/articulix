...
import { Gift, Plus, RefreshCw, CheckCircle, Clock, Truck } from 'lucide-react'
...
async function enregistrerCA(sachetsCount: number) {
  ...
  if (existing) {
    ...
    await supabase.from('monthly_revenue').insert({
      month, year, sachets_sold: sachetsCount, revenue
    })
  }
}
    ...
    <div className="min-h-screen">
      ...
      <main className="max-w-5xl mx-auto px-6 py-8">
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-12 text-center text-gray-400">
            <RefreshCw className="w-10 h-10 mx-auto mb-3 opacity-30" />
            <p className="text-sm">Aucun réassort. Planifiez votre premier réassort !</p>
          </div>
        ) : (
          <div className="space-y-3">
            {filtered.map(r => {
