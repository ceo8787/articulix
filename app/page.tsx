...
export default function Dashboard() {
  ...
  async function load() {
    const [{ data: m }, { data: v }, { data: r }, { data: f }, { data: t }] = await Promise.all([
  ...
  const totalFigurines = models.reduce((acc, m) => acc + m.stock_normal + m.stock_gold, 0)
  ...
  // CA estimé : on suppose stock de départ = sachets_target, vendus = target - current
  const caEstime = venues.reduce((acc, v) => {
    const vendus = Math.max(0, v.sachets_target - v.sachets_current)
    return acc + vendus * 5
  }, 0)
                ...
                <div className="space-y-1">
                  {criticalFilaments.map(f => (
                    <div key={f.id} className="flex items-center justify-between">
                      ...
                      <span className="text-xs px-2 py-0.5 rounded-full bg-red-100 text-red-700 font-medium">🔴 {f.weight_remaining}g restant</span>
                    </div>
                  ))}
                  {lowFilaments.map(f => (
                    <div key={f.id} className="flex items-center justify-between">
                      <span className="text-sm text-amber-700">{f.brand} — {f.color}</span>
                      <span className="text-xs px-2 py-0.5 rounded-full bg-amber-100 text-amber-700 font-medium">🟡 {f.weight_remaining}g restant</span>
                    </div>
                  ))}
                </div>
              </div>
  ...
  async function loadStats() {
    ...
    const low = (models || []).filter(m => { const t = m.stock_normal + m.stock_gold; return t >= 10 && t < 20 }).length
    const pending = (reorders || []).filter(r => r.status === 'planifié').length
    const total = (models || []).reduce((acc, m) => acc + m.stock_normal + m.stock_gold, 0)
    setStats({ totalModels: (models || []).length, urgentModels: urgent, lowModels: low, totalVenues: (venues || []).length, pendingReorders: pending, totalFigurines: total })
  ...
  const statCards = [
    ...
    { label: 'Figurines en stock', value: stats.totalFigurines, icon: Gift, color: 'bg-green-50 text-green-700', href: '/models' },
  ]
