          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 mb-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="col-span-2">
                ...
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" placeholder="Ex : Intermarché Panazol" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} />
              </div>
              <div>
              ...
              <div>
                ...
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={form.contact} onChange={e => setForm(f => ({ ...f, contact: e.target.value }))} />
              </div>
              <div>
                <label className="text-xs text-gray-500 block mb-1">Objectif sachets</label>
              ...
              <div>
                <label className="text-xs text-gray-500 block mb-1">Jour du réassort mensuel</label>
                <input type="number" min="1" max="28" className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" placeholder="Ex: 15" value={form.reorder_day} onChange={e => setForm(f => ({ ...f, reorder_day: +e.target.value }))} />
              </div>
              <div className="col-span-2">
                <label className="text-xs text-gray-500 block mb-1">Notes</label>
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
              </div>
            </div>
                ...
                <div key={v.id} className="bg-white rounded-xl border border-gray-100 p-5">
                  <div className="flex items-start justify-between mb-3">
                    <div>
                      ...
                      {days !== null && (
                        <p className={`text-xs mt-1 flex items-center gap-1 ${days <= 7 ? 'text-amber-600 font-medium' : 'text-gray-400'}`}>
                          <RefreshCw className="w-3 h-3" />
                          Réassort le {v.reorder_day} du mois — {days === 0 ? "aujourd'hui !" : `dans ${days} jour(s)`}
                        </p>
                      )}
                    </div>
                    <div className="flex items-center gap-2">
                      <button onClick={() => planifierReassort(v)} className="text-xs px-3 py-1.5 bg-brand-light text-brand-dark rounded-lg hover:bg-brand hover:text-white transition-colors flex items-center gap-1">
                        <RefreshCw className="w-3 h-3" /> Planifier réassort
                      </button>
