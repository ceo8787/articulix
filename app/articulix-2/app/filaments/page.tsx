...
export default function FilamentsPage() {
  ...
  const urgent = filaments.filter(f => status(f) === 'urgent').length
  const low = filaments.filter(f => status(f) === 'low').length
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            ...
            <div className="flex gap-2 mt-1">
              {urgent > 0 && <span className="text-xs px-2 py-0.5 bg-red-50 text-red-700 rounded-full">{urgent} bobine(s) presque vide</span>}
              {low > 0 && <span className="text-xs px-2 py-0.5 bg-amber-50 text-amber-700 rounded-full">{low} bobine(s) à surveiller</span>}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 mb-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" placeholder="Bambu Lab, Polymaker..." value={form.brand} onChange={e => setForm(f => ({ ...f, brand: e.target.value }))} />
              </div>
              <div>
              ...
              <div className="col-span-3">
                <label className="text-xs text-gray-500 block mb-1">Notes (référence, lien...)</label>
                <input className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm" placeholder="Ex: SKU-12345, commandé sur Amazon..." value={form.notes} onChange={e => setForm(f => ({ ...f, notes: e.target.value }))} />
              </div>
            </div>
